...
<div class="text">
    <v-dialog v-model="dialog" max-width="500px">
        <template v-slot:activator="{ on, attrs }">
            <v-btn color="orange lighten-1 " class="black--text buttonCreate" dark v-bind="attrs" v-on="on" max-width="140px" v-if="userAccount.role !='Student'">
                +Permission
            </v-btn>
        </template>
        <v-card>
                    ...
                    <v-row>
                        <v-col class="d-flex" cols="12" sm="12" md="12">
                            ...
                            <select name="student" id="student" class="select_student" v-model="id">
                                <option v-for="stu of students" :key="stu.id" :value="stu.id">
                                    {{ stu.first_name }}
                                    {{ stu.last_name }}
                                </option>
                            </select>
                        ...
                        <v-col class="d-flex" cols="12" sm="12">
                            <v-select v-model="reason" :items="permissionType" label="Choose leave type" required></v-select>
                        </v-col>
                        <v-col cols="12" md="12">
                            <v-textarea solo name="input-7-4" v-model="description" label="Description..."></v-textarea>
                        </v-col>
                    </v-row>
                </v-container>
            </v-card-text>
