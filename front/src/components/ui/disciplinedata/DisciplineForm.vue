...
<div class="text">
    <v-dialog v-model="dialog" max-width="500px">
        <template v-slot:activator="{ on, attrs }">
            <v-btn color="orange lighten-1 " class="black--text createButton" dark v-bind="attrs" v-on="on" v-if="userAccount.role ==='admin'">
                +Discipline
            </v-btn>
        </template>
        <v-card>
                ...
                <v-container>
                    <v-row>
                        <v-col class="d-flex" cols="12" sm="12" md="12">
                            ...
                            <select :error-messages="studentErrors" required @input="$v.student_id.$touch()" @blur="$v.student_id.$touch()" name="student" id="student" class="select_student" v-model="id">
                                <option v-for="(student, index) of students" :key="index" :value="student.id">
                                    {{ student.first_name }}
                                    {{ student.last_name }}
                                </option>
                            </select>
                        ...
                        <v-col cols="12" sm="12" md="12">
                            <v-text-field :error-messages="dateErrors" required @input="$v.date.$touch()" @blur="$v.date.$touch()" type="date" v-model="date"> </v-text-field>
                        </v-col>
                        <v-col class="d-flex" cols="12" sm="12">
                            <v-select :error-messages="typeErrors" required @input="$v.type.$touch()" @blur="$v.type.$touch()" v-model="type" :items="disciplineType" label="Choose leave type"></v-select>
                        </v-col>
                        <v-col cols="12" md="12">
                            <v-textarea :error-messages="explainationErrors" required @input="$v.explaination.$touch()" @blur="$v.explaination.$touch()" solo name="input-7-4" v-model="explaination" label="Description..."></v-textarea>
                        </v-col>
                    </v-row>
                </v-container>
            </v-card-text>
