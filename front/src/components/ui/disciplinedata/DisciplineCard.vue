...
<v-container>
    <template>
        ...
        <div class="discipline">
            <v-card color="grey lighten-4" class="mx-auto card" max-width="2000" max-height="130" v-for="(discipline, index) in datadiscipline" :key="index">
                <v-row>
                    ...
                    <v-col cols="12" sm="2" >
                        <div class="i_con mt-4">
                            <v-icon color="blue darken-1" text size="30px" v-if="userAccount.role === 'admin'">mdi-lead-pencil</v-icon>
                            <v-icon color="red darken-1" text size="30px" @click="getPermissionId(permission.id)" v-if="userAccount.role === 'admin'">mdi-delete</v-icon><br /><br /><br />
                            <v-col cols="auto" >
                                <v-dialog transition="dialog-top-transition" max-width="550" >
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-btn color="white" class="blue--text detailButton" v-bind="attrs" v-on="on">
                                    ...
                                    <template v-slot:default="dialog">
                                        <v-card>
                                            <v-toolbar color="blue lighten-4" darkclass="text-h4 pa-7">
                                                <v-list-item-avatar color="grey darken-3">
                                                    <v-img class="elevation-6" alt="">
                                                        <v-icon color="orange" size="60px"> mdi-alert </v-icon>
                                                    </v-img>
                                                </v-list-item-avatar>
                                            ...
                                            <v-card-actions class="justify-end">
                                                <v-btn color="blue" text @click="dialog.value = false">Close</v-btn>
                                            </v-card-actions>
                                        </v-card>
                                    </template>
                                </v-dialog>
